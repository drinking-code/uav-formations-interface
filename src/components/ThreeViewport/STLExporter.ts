// patched. from https://github.com/pmndrs/three-stdlib/blob/main/src/exporters/STLExporter.ts
...
class STLExporter<ExporterOptions extends STLExporterOptions> {
    private binary

    private output: DataView | string
    private offset
    ...
    public parse(scene: Object3D, options?: ExporterOptions): (ExporterOptions['binary'] extends true ? DataView : string)
    public parse(scene: Object3D, options?: ExporterOptions): DataView | string {
        ...
        for (let i = 0, il = this.objects.length; i < il; i++) {
            ...
            if (index !== null) {
                ...
                for (let j = 0; j < index.count; j += 3) {
                    ...
                    this.writeFace(
                        a, b, c,
                        positionAttribute as BufferAttribute | InterleavedBufferAttribute,
                        object as Mesh | SkinnedMesh
                    )
                }
            } else {
